...
char lcd_text[20];
uint8_t lcd_num = 0;
float A_fre, B_fre, A_tim, B_tim;
bool is_fre = 1;	// 显示模式，1：频率显示
int32_t PD = 1000, PH = 5000;
int32_t PX = 0;
uint32_t NDA, NDB, NHA, NHB;

extern uint8_t is_select;
extern float freA, freB;
void lcd_task(void)
{
	switch(lcd_num)
	{
		case 0:
			...
			if(is_fre)
			{
				...
				{
					A_fre = freA / 1000.0f;
					sprintf(lcd_text, "     A=%.2fKHz     ", A_fre);
					LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
				}
				else if((freA <= 1000) && (freA >= 0))
				{
					A_fre = freA;
					sprintf(lcd_text, "     A=%dHz        ", (uint32_t)A_fre);
					LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
				}
				else if(freA < 0)
				{
					sprintf(lcd_text, "     A=NULL        ");
					LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
				}
				...
				{
					B_fre = freB / 1000.0f;
					sprintf(lcd_text, "     B=%.2fKHz     ", B_fre);
					LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
				}
				else if((freB <= 1000) && (freB >= 0))
				{
					B_fre = freB;
					sprintf(lcd_text, "     B=%dHz        ", (uint32_t)B_fre);
					LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
				}
				else if(freB < 0)
				{
					sprintf(lcd_text, "     B=NULL        ");
					LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
				}
			}
			else
			{
				...
				if(freA != 0)
				{
					A_tim = 1000000.0f / freA;	// us为单位
				}
				
				if(freB != 0)
				{
					B_tim = 1000000.0f / freB;
				}
				
				if(A_tim > 1000)
				{
					A_tim = A_tim / 1000.0f;
					sprintf(lcd_text, "     A=%.2fmS       ", A_tim);
					LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
				}
				else if((A_tim <= 1000) && (A_tim >= 0))
				{
					A_tim = A_tim;
					sprintf(lcd_text, "     A=%duS        ", (uint32_t)A_tim);
					LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
				}
				else if(A_tim < 0)
				{
				...
				{
					B_tim = B_tim / 1000.0f;
					sprintf(lcd_text, "     B=%.2fmS       ", B_tim);
					LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
				}
				else if((B_tim <= 1000) && (B_tim >= 0))
				{
					B_tim = B_tim;
					sprintf(lcd_text, "     B=%duS        ", (uint32_t)B_tim);
					LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
				}
				else if(B_tim < 0)
				{
		...
		case 1:
            ...
            LCD_DisplayStringLine(Line1, (unsigned char *)lcd_text);		
			sprintf(lcd_text, "     PD=%dHz      ", PD);
            LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
			sprintf(lcd_text, "     PH=%dHz      ", PH);
            LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
			sprintf(lcd_text, "     PX=%dHz      ", PX);
            LCD_DisplayStringLine(Line5, (unsigned char *)lcd_text);
			sprintf(lcd_text, "                    ");
            LCD_DisplayStringLine(Line6, (unsigned char *)lcd_text);
		break;
		case 2:
			sprintf(lcd_text, "        RECD        ");
            LCD_DisplayStringLine(Line1, (unsigned char *)lcd_text);
			sprintf(lcd_text, "     NDA=%d          ", NDA);
            LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
			sprintf(lcd_text, "     NDB=%d          ", NDB);
            LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
			sprintf(lcd_text, "     NHA=%d          ", NHA);
            LCD_DisplayStringLine(Line5, (unsigned char *)lcd_text);
			sprintf(lcd_text, "     NHB=%d          ", NHB);
            LCD_DisplayStringLine(Line6, (unsigned char *)lcd_text);
		break;
	}
}