...
uint8_t ST;				// 0:idle, 1:busy, 2:wait
int16_t CP[2] = {0};	// 当前位置
int16_t TP[2] = {0};	// 下一个位置
float SE;				// 运行状态下的速度值
uint32_t RN;			// 剩余途径地
float R = 1.0f;				// 可调参数
uint32_t B = 10;				// 可调参数
float TS;				// 累计行程
uint32_t TT;			// 累计运行时长，不计空闲、等待时间
void lcd_task(void)
{
	switch(lcd_num)
	{
		case 0:
			...
			{
				case 0:
					sprintf(lcd_text, "     ST=Idle        ");
					LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
				break;
				case 1:
					sprintf(lcd_text, "     ST=Busy        ");
					LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
				break;
				case 2:
					sprintf(lcd_text, "     ST=Wait        ");
					LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);
				break;
				default:
				break;
		...
		case 1:
			...
			sprintf(lcd_text, "     R=%.1f          ", R);
			LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);

			sprintf(lcd_text, "     B=%d           ", B);
			LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
		...
		case 2:
			...
			sprintf(lcd_text, "     TS=%.1f          ", TS);
			LCD_DisplayStringLine(Line3, (unsigned char *)lcd_text);

			sprintf(lcd_text, "     TT=%d           ", TT);
			LCD_DisplayStringLine(Line4, (unsigned char *)lcd_text);
