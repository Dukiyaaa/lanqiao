...
uint8_t show_num = 0;
char show_buffer[20] = {0};
float PA4, PA5;
extern float pa1_fre;
uint8_t X = 1, Y = 1;
uint8_t pa_num = 4;
uint32_t N, N_4, N_5;
float A, T, H;
float A_4, A_5, T_4 , T_5 , H_4, H_5;
uint8_t lcd_mode = 0;	// 0:正放
...
void show_task(void)
{
	...
	switch(show_num)
	{
		case 0:
			...
			sprintf(show_buffer, "     PA4=%.2f       ", PA4);
			LCD_DisplayStringLine(Line3, (unsigned char *)show_buffer);
			sprintf(show_buffer, "     PA5=%.2f       ", PA5);
			LCD_DisplayStringLine(Line4, (unsigned char *)show_buffer);
			sprintf(show_buffer, "     PA1=%d         ", (uint32_t)pa1_fre);
			LCD_DisplayStringLine(Line5, (unsigned char *)show_buffer);
			sprintf(show_buffer, "                     ");
			LCD_DisplayStringLine(Line6, (unsigned char *)show_buffer);
			sprintf(show_buffer, "                     ");
			LCD_DisplayStringLine(Line8, (unsigned char *)show_buffer);
		break;
		case 1:
			LCD_DisplayStringLine(Line1, (unsigned char *)"        PARA        ");
			sprintf(show_buffer, "     X=%d           ", X);
			LCD_DisplayStringLine(Line3, (unsigned char *)show_buffer);
			sprintf(show_buffer, "     Y=%d           ", Y);
			LCD_DisplayStringLine(Line4, (unsigned char *)show_buffer);
			sprintf(show_buffer, "                     ");
			LCD_DisplayStringLine(Line5, (unsigned char *)show_buffer);
			sprintf(show_buffer, "                     ");
		...
		break;
		case 2:
			sprintf(show_buffer, "        REC-PA%d     ", pa_num);
			LCD_DisplayStringLine(Line1, (unsigned char *)show_buffer);
			if(pa_num == 4)
			{
				N = N_4;
				A = A_4;
			...
			sprintf(show_buffer, "     N=%d           ", N);
			LCD_DisplayStringLine(Line3, (unsigned char *)show_buffer);
			sprintf(show_buffer, "     A=%.2f         ", A);
			LCD_DisplayStringLine(Line4, (unsigned char *)show_buffer);
			sprintf(show_buffer, "     T=%.2f        ", T);
			LCD_DisplayStringLine(Line5, (unsigned char *)show_buffer);
			sprintf(show_buffer, "     H=%.2f        ", H);
			LCD_DisplayStringLine(Line6, (unsigned char *)show_buffer);
			sprintf(show_buffer, "                     ");
			LCD_DisplayStringLine(Line8, (unsigned char *)show_buffer);
		break;
